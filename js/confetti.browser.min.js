...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.confetti-card').forEach((el) => {
        const observer = new IntersectionObserver(([entry]) => {
            if (entry.isIntersecting && !triggered.has(el)) {
                triggered.add(el);

                // ✅ Delay the confetti animation by 1000ms after card becomes visible
                setTimeout(() => {
                ...
                }, 1000); // <-- 1 second delay after visibility
            }
        }, { threshold: 0.6 });
